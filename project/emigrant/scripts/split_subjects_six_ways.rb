...
SPLIT_OVERLAP_VERT      = 0.01                # When splitting pages cut each side so that it bleeds this 
                                              ...
                                              #   midline)
SPLIT_OVERLAP_HORIZ     = 0.02                # When splitting pages cut each side so that it bleeds this 
...
$in_path = "#{File.dirname(File.dirname(__FILE__))}/subjects/subjects_from_api.book14.csv"
$out_path = "#{File.dirname(File.dirname(__FILE__))}/subjects/group_only_one_group.book14.csv"
...
$rows = []
$rows = CSV.read($out_path, headers: true).map { |r| r.to_h } if File.exist? $out_path
...
def make_crops(img, row)
  ...
  height = img.rows * SPLIT_POSITION_VERT
  y_positions = [0, height, (2 * height)]
  ...
  if img.columns > img.rows
    ...
    # Left-top:
    coords = { x: 0, y: y_positions[0], w: width, h: height + v_overlap }
    make_crop img, row, 'left-top', coords

    # Left-middle
    coords = { x: 0, y: y_positions[1] - v_overlap, w: width, h: height + 2*v_overlap }
    make_crop img, row, 'left-middle', coords
    
    # Left-bottom:
    coords = { x: 0, y: y_positions[2] - v_overlap, w: width, h: height + v_overlap }
    make_crop img, row, 'left-bottom', coords

    # Right cuts:

    right = img.columns / 2 - h_overlap

    # Right-top:
    coords = { x: right, y: y_positions[0], w: width, h: height + v_overlap }
    make_crop img, row, 'right-top', coords

    # Right-middle
    coords = { x: right, y: y_positions[1] - v_overlap, w: width, h: height + 2*v_overlap }
    make_crop img, row, 'right-middle', coords
    
    # Right-bottom:
    coords = { x: right, y: y_positions[2] - v_overlap, w: width, h: height + v_overlap }
    ...
    # Left-middle
    coords = { x: 0, y: y_positions[1] - v_overlap, w: width, h: height + 2*v_overlap }
    make_crop img, row, 'unsplit-middle', coords
...
puts "Fetching list of existing objects..."
...
CSV.foreach($in_path, headers: true) do |row| 
  ...
  # next if ! RECUT && ! $rows.select { |r| r['capture_uuid'] == row['capture_uuid'] }.empty?
